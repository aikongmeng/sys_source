...
从上往下大致浏览一下这类，1k来行 ， 就个人喜爱来挨个看看Android工程师是如何搞这个类的。
依据Java的加载顺序，首先，来看static的方法，是一个私有的静态方法`SystemClassLoader` 
```java
...
```
看到注释了`URLClassLoader`也凑一下这个类，[在线源码](http://androidxref.com/9.0.0_r3/xref/libcore/ojluni/src/main/java/java/net/URLClassLoader.java)  文档显示大意为： 用于从指向 JAR 文件和目录的 URL 的搜索路径加载类和资源。 
代码中用`PathClassLoader`代替了`URLClassLoader`。
[PathClassLoader](http://androidxref.com/9.0.0_r3/xref/libcore/dalvik/src/main/java/dalvik/system/PathClassLoader.java)
作用就是
 `Android uses this class for its system class
